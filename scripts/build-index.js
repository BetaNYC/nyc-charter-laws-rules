#!/usr/bin/env node
// Parses the three AML bulk XML ZIPs and builds a searchable JSON index.
...
for (const corpus of CORPORA) {
  ...
  writeFileSync(
    join(INDEX_DIR, `${corpus.key}.json`),
    JSON.stringify(sections, null, 2)
  );
  console.log(`  Saved data/index/${corpus.key}.json`);
}

writeFileSync(join(INDEX_DIR, "versions.json"), JSON.stringify(versions, null, 2));
